...
@Component({
  selector: 'apartment-system-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <apartment-system-country-orders-map (select)="selectCountryById($event)"
                                countryId="USA">
        </apartment-system-country-orders-map>
        <apartment-system-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  maxValue="20">
        </apartment-system-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
